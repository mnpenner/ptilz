...
class MyPdo extends PdoPlus {
    ...
    public function prepare_insert($table, $columns, $options = 0) {
        ...
        $column_sql = implode(', ', array_map(array('self', 'quote_identifier'), $columns));
        $placeholder_arr = array();
        foreach($columns as $col) {
            $placeholder_arr[] = ":$col";
        }
        $placeholder_sql = implode(', ', $placeholder_arr);
        $sql = ($options & self::REPLACE) ? 'REPLACE ' : 'INSERT ';
        if($options & self::LOW_PRIORITY) $sql .= 'LOW_PRIORITY ';
        elseif($options & self::DELAYED) $sql .= 'DELAYED ';
        elseif($options & self::HIGH_PRIORITY) $sql .= 'HIGH_PRIORITY ';
        if($options & self::IGNORE) $sql .= 'IGNORE ';
    ...
    public function copy_table($table, $from_db, $to_db, $truncate=true, $where=null) {
        ...
        $sql = "INSERT INTO $to_db_esc.$tbl_esc ($columns) SELECT $columns FROM $from_db_esc.$tbl_esc";
    ...
    const ER_FILE_CORRUPT = 1875;
    const ER_ERROR_ON_MASTER = 1876;
    const ER_INCONSISTENT_ERROR = 1877;
    const ER_STORAGE_ENGINE_NOT_LOADED = 1878;
    const ER_GET_STACKED_DA_WITHOUT_ACTIVE_HANDLER = 1879;
    const ER_WARN_LEGACY_SYNTAX_CONVERTED = 1880;
    const ER_BINLOG_UNSAFE_FULLTEXT_PLUGIN = 1881;
    const ER_CANNOT_DISCARD_TEMPORARY_TABLE = 1882;
    const ER_FK_DEPTH_EXCEEDED = 1883;
    const ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2 = 1884;
    const ER_WARN_TRIGGER_DOESNT_HAVE_CREATED = 1885;
    const ER_REFERENCED_TRG_DOES_NOT_EXIST = 1886;
}
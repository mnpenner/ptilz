...
class Sql {
    ...
    public static function format($query, $params = []) {
        return preg_replace_callback('~(?|(\?{1,2})|(:{1,2})([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*))~', function ($matches) use (&$params) {
            switch($matches[1]) {
                case '?': return Sql::quote(array_shift($params));
                case '??': return Sql::escapeId(array_shift($params));
                case ':': return Sql::quote($params[$matches[2]]);
                case '::': return Sql::escapeId($params[$matches[2]]);
            }
